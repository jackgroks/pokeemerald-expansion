...
static void Task_GiveExpToMon(u8 taskId)
{
    ...
    {
        struct Pokemon *mon = &GetBattlerParty(battler)[monId];
...
static void Task_PrepareToGiveExpWithExpBar(u8 taskId)
{
    ...
    s32 gainedExp = GetTaskExpValue(taskId);
    enum BattlerId battler = gTasks[taskId].tExpTask_battler;
    struct Pokemon *mon = &GetBattlerParty(battler)[monIndex];
    u8 level = GetMonData(mon, MON_DATA_LEVEL);
    u16 species = GetMonData(mon, MON_DATA_SPECIES);
...
static void Task_UpdateLvlInHealthbox(u8 taskId)
{
    ...
    if (!gBattleSpritesDataPtr->healthBoxesData[battler].specialAnimActive)
    {
        ...
        if (IsDoubleBattle() == TRUE && monIndex == gBattlerPartyIndexes[BATTLE_PARTNER(battler)])
            UpdateHealthboxAttribute(gHealthboxSpriteIds[BATTLE_PARTNER(battler)], &GetBattlerParty(BATTLE_PARTNER(battler))[monIndex], HEALTHBOX_ALL);
        else
            UpdateHealthboxAttribute(gHealthboxSpriteIds[battler], &GetBattlerParty(battler)[monIndex], HEALTHBOX_ALL);
...
void PlayerHandleExpUpdate(enum BattlerId battler)
{
    ...
    if (GetMonData(&GetBattlerParty(battler)[monId], MON_DATA_LEVEL) >= MAX_LEVEL)
    {
        BtlController_Complete(battler);
    }
    else
