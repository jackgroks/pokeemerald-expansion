// SPDX-License-Identifier: (engine: GPL-2.0-or-later from upstream pe-expansion)
//
// trade_pe_templates.h — pe-Hoenn-side BG/window/sprite/text/template statics
// restored as a Phase 2 minimal-strip additive include.
//
// Background: Phase 3's T8 migration tool (tools/migration/t8_region_heal_trade.py)
// emitted src/data/trade.h containing HnS's sIngameTrades[] but DROPPED
// pe-master's accompanying static template data that src/trade.c references
// (BgTemplate / WindowTemplate / SpriteTemplate / animation / text / coord
// tables). Without these, trade.o fails to compile with ~70 undefined
// references.
//
// This file is a verbatim copy of pe-master's src/data/trade.h with the
// sIngameTrades[] block removed (HnS supplies its own via src/data/trade.h).
// It must be #include'd in src/trade.c BEFORE "data/trade.h" so the templates
// are available when the HnS sIngameTrades block is parsed.
//
// Tool-fix follow-up queued: extend T8 emission to preserve pe template data
// when landing HnS trade data. Until that lands, this additive restoration
// is the agreed strategy α for the Phase 2 minimal-strip close-out.
//
// Source: pokeemerald-expansion master @ rh-hideout/pokeemerald-expansion
//         src/data/trade.h (1380 lines; lines 969-1215 omitted here).
...
// --- sIngameTrades[] omitted (lines 969-1215 of pe-master trade.h) ---
// HnS supplies its own sIngameTrades[] via src/data/trade.h.
